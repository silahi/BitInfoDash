...
/**
 * Performance test for the BitcoinOverview entity.
 */
public class BitcoinOverviewGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all bitcoinOverviews").get("/api/bitcoin-overviews").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new bitcoinOverview")
                        .post("/api/bitcoin-overviews")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_bitcoinOverview_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created bitcoinOverview").get("${new_bitcoinOverview_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created bitcoinOverview").delete("${new_bitcoinOverview_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BitcoinOverview entity").exec(scn);
