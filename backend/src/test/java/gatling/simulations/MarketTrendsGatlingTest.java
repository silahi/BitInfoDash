...
/**
 * Performance test for the MarketTrends entity.
 */
public class MarketTrendsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all marketTrends").get("/api/market-trends").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new marketTrends")
                        .post("/api/market-trends")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"trendName\": \"SAMPLE_TEXT\"" +
                                ", \"indicatorValue\": 0" +
                                ", \"timestamp\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"trendType\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_marketTrends_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created marketTrends").get("${new_marketTrends_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created marketTrends").delete("${new_marketTrends_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MarketTrends entity").exec(scn);
