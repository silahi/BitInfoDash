...
/**
 * Performance test for the OHLCVT12h entity.
 */
public class OHLCVT12hGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all oHLCVT12hs").get("/api/ohlcvt-12-hs").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new oHLCVT12h")
                        .post("/api/ohlcvt-12-hs")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_oHLCVT12h_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created oHLCVT12h").get("${new_oHLCVT12h_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created oHLCVT12h").delete("${new_oHLCVT12h_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OHLCVT12h entity").exec(scn);
