...
/**
 * Performance test for the BitcoinAddress entity.
 */
public class BitcoinAddressGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all bitcoinAddresses").get("/api/bitcoin-addresses").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created bitcoinAddress").get("${new_bitcoinAddress_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created bitcoinAddress").delete("${new_bitcoinAddress_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BitcoinAddress entity").exec(scn);
