 ...
 */
public class TransactionsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all transactions").get("/api/transactions").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new transactions")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"senderAddress\": \"SAMPLE_TEXT\"" +
                                ", \"recipientAddress\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created transactions").get("${new_transactions_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created transactions").delete("${new_transactions_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Transactions entity").exec(scn);
