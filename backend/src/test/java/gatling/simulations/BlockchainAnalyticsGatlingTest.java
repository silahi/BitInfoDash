...
/**
 * Performance test for the BlockchainAnalytics entity.
 */
public class BlockchainAnalyticsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all blockchainAnalytics")
                ...
                .exec(
                    http("Create new blockchainAnalytics")
                        .post("/api/blockchain-analytics")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"hashrateDistribution\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_blockchainAnalytics_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created blockchainAnalytics").get("${new_blockchainAnalytics_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created blockchainAnalytics").delete("${new_blockchainAnalytics_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BlockchainAnalytics entity").exec(scn);
