...
/**
 * Performance test for the OHLCVT1m entity.
 */
public class OHLCVT1mGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all oHLCVT1ms").get("/api/ohlcvt-1-ms").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new oHLCVT1m")
                        .post("/api/ohlcvt-1-ms")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_oHLCVT1m_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created oHLCVT1m").get("${new_oHLCVT1m_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created oHLCVT1m").delete("${new_oHLCVT1m_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OHLCVT1m entity").exec(scn);
