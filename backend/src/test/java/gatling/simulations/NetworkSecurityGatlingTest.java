...
/**
 * Performance test for the NetworkSecurity entity.
 */
public class NetworkSecurityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all networkSecurities").get("/api/network-securities").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new networkSecurity")
                        .post("/api/network-securities")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"resolution\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_networkSecurity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created networkSecurity").get("${new_networkSecurity_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created networkSecurity").delete("${new_networkSecurity_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the NetworkSecurity entity").exec(scn);
