...
/**
 * Performance test for the OHLCVT1d entity.
 */
public class OHLCVT1dGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all oHLCVT1ds").get("/api/ohlcvt-1-ds").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new oHLCVT1d")
                        .post("/api/ohlcvt-1-ds")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_oHLCVT1d_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created oHLCVT1d").get("${new_oHLCVT1d_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created oHLCVT1d").delete("${new_oHLCVT1d_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OHLCVT1d entity").exec(scn);
