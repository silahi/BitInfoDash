import { Component } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';

import SharedModule from 'app/shared/shared.module';
import { ITEM_DELETED_EVENT } from 'app/config/navigation.constants';
import { IBlockchainAnalytics } from '../blockchain-analytics.model';
import { BlockchainAnalyticsService } from '../service/blockchain-analytics.service';

@Component({
  standalone: true,
  templateUrl: './blockchain-analytics-delete-dialog.component.html',
  imports: [SharedModule, FormsModule],
})
export class BlockchainAnalyticsDeleteDialogComponent {
  blockchainAnalytics?: IBlockchainAnalytics;

  constructor(
    protected blockchainAnalyticsService: BlockchainAnalyticsService,
    protected activeModal: NgbActiveModal,
  ) {}

  cancel(): void {
    this.activeModal.dismiss();
  }

  confirmDelete(id: number): void {
    this.blockchainAnalyticsService.delete(id).subscribe(() => {
      this.activeModal.close(ITEM_DELETED_EVENT);
    });
  }
}
